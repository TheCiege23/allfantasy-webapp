...
export default function PlayerFinderClient({ defaultUsername = '' }: { defaultUsername?: string }) {
  ...
  async function handleSearch() {
    ...
    try {
      const res = await fetch('/api/legacy/player-finder', {
        ...
        body: JSON.stringify({ sleeper_username: normalizedUsername, query: normalizedQuery }),
      })
  ...
  return (
    <div className="min-h-screen bg-[#0a051f] text-white p-6 md:p-10 space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <input
          className="bg-white/5 border border-white/20 rounded px-3 py-2"
        ...
        <button
          ...
          disabled={loading || !username.trim() || query.trim().length < 2}
          className="bg-cyan-600 rounded px-3 py-2 disabled:opacity-50"
        >
