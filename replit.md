...
-   **2-Stage Trade Analysis Pipeline**: Stage A (Deterministic Intelligence Layer) assembles a canonical `TradeDecisionContextV1` object (Zod-validated, `trade-decision-context.ts`) from league settings, roster composition/needs, trade history (recency-weighted), manager preference vectors/tendencies, asset valuations (with source + timestamp), player risk markers (age curve buckets, injury recency), competitor team snapshots, and missing-data flags. Stage B (Peer Review Layer) uses `runPeerReviewAnalysis()` to send the identical fact layer to OpenAI and Grok as peer reviewers with the same prompt contract (`PEER_REVIEW_PROMPT_CONTRACT`), same schema validator (`PeerReviewVerdictSchema`), and same temperature/maxTokens. Each provider returns `{ verdict, confidence, reasons[], counters[], warnings[] }`. Merge logic: if both agree on verdict class → boost confidence (+10); if they disagree → return "Disagreement" verdict with capped confidence (max 40); if one fails → degrade gracefully to the other provider with a -10 confidence penalty. Implemented in `trade-analysis-schema.ts` (schema + merge), `dual-brain-trade-analyzer.ts` (provider calls), and `dynasty-trade-analyzer/route.ts` (API endpoint returning both `peerReview` and backward-compatible `analysis` fields).

**3-Section Trade Response (`trade-response-formatter.ts`):**
The API response is structured into 3 user-facing sections: (1) **Value Verdict** — fairness grade (A+ to F), edge label, value delta, confidence, veto risk, and AI reasoning; (2) **Viability Verdict** — acceptance likelihood, partner fit score (roster needs/surplus alignment), timing fit (contender/rebuild window alignment), league activity, and acceptance signals; (3) **Action Plan** — best offer assessment (send-as-is vs needs adjustment), up to 2 counter proposals, and a suggested message text for the trade partner. The `sections` field is additive alongside the backward-compatible `analysis` field.
