...
-   **Player Media System**: Standardized player headshot and team logo delivery via `lib/player-media.ts`. `buildPlayerMedia(playerId, teamAbbr)` returns `{ headshotUrl, teamLogoUrl }` using Sleeper CDN for headshots and ESPN for team logos. All three core AI endpoints (Trade Partner Matchmaking, Waiver AI, Trade Evaluator) enrich response objects with `playerId`, `fullName`, `teamAbbr`, `sport`, and `media` fields. Frontend never derives media URLs. Centralized resolver chain: `attachPlayerMedia(player)` and `attachTeamMedia(teamAbbr, sport)` use DB-first lookup (PlayerIdentityMap → SportsPlayer) with URL template fallback. `attachPlayerMediaBatch(players[])` performs batched DB queries to avoid N+1. In-memory TTL cache (6h) for resolved media. `PlayerTeamHistory` model (`player_team_history` table) stores historical team affiliations by season/week for correct logo rendering on past trades. `getHistoricalTeam()` and `attachPlayerMediaHistorical()` resolve team at a specific point in time. `recordTeamHistory()` upserts entries. Week 0 = season-level entry.
-   **Stable User Identity**: `sleeperUser` identity object `{ username, userId }` standardized across Trade Partner Matchmaking, Dynasty Trade Evaluator, and Waiver AI endpoints. `buildSleeperUser()` helper provides consistent identity caching (resolve once, store both fields). Optional with full backwards compatibility for legacy username-only requests. Server preferentially uses `userId` over `username` when available.
