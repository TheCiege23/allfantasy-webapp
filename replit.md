...
-   **ESPN API**: Enhanced multi-endpoint integration via `lib/espn-data.ts`. Provides: live scores (`/scoreboard`), news (`/news` with team filtering), team rosters (`/teams/{id}?enable=roster` — full player directory with headshots, position, experience, college), injuries (`/teams/{id}?enable=injuries` — per-team injury reports synced to `sports_injuries` table), game summaries/box scores (`/summary?event={id}` — player game stats, drive counts), and standings. Roster sync runs daily as medium-priority background task. Injuries synced in parallel with API-Sports. ESPN serves as quaternary fallback for teams/games/standings in the sports router cascade, but is primary source for live scores, news, and now injuries+rosters. Circuit breaker protected.
...
-   **FantasyCalc API**: Primary trade value source using two endpoints: `/values/current` (447 valued players with dynasty/redraft values, tiers, ADP, trade frequency, volatility) and `/players` (full 3,853-player directory with cross-platform IDs: Sleeper, ESPN, MFL, Fleaflicker). New fields captured: `espnId`, `fleaflickerId`, `maybeTier`, `maybeAdp`, `maybeTradeFrequency`, `maybeMovingStandardDeviationAdjusted` (volatility). Player directory enables cross-platform identity resolution via `PlayerIdentityMap` (now includes `espnId` and `fleaflickerId` columns).
