...
-   **League Rankings V2 (Team-Level)**: A two-layer team ranking system (Power Rankings and Dynasty Outlook) with five core scores, AI Coach Box, Motivational Framing System, Luck Meter, and Win Window projection. Features a rich per-player injury model with recency decay, severity, uncertainty, and risk concentration (replacing flat penalties); DB injury enrichment from `SportsInjury` table; per-player injury discounts in market value (25% dynasty, 60% redraft); a backtest engine (`lib/rankings-engine/backtest.ts`) with truth targets (win_pct_3w, playoff_qual, championship_finish) and accuracy metrics (Brier, ECE, NDCG, Spearman); a composite parameter learning module (`lib/rankings-engine/composite-param-learning.ts`) with grid search optimization for injuryInfluence, starterBenchSplit, luckDampening, futureCapitalInfluence; adaptive weights fetched from `RankingWeightsWeekly` DB table that modify composite weight profiles and power score parameters (with static `composite-weights.json` as fallback); data-driven draft gain percentile (computed from draft picks vs expected slot values); and a calibratable composite weight config system. Each team includes a `dataQuality` object with `rankingConfidence` (0-100 score with HIGH/MEDIUM/LOW rating), `dataCoverage` (FULL/PARTIAL/MINIMAL tier), `stalenessHours` (per-source: injury from DB records, valuation from FC cache age, sleeper from DB sync timestamp), and human-readable `signals` explaining confidence deductions. An anti-gaming constraint layer (`lib/rankings-engine/anti-gaming.ts`) prevents unjustified rank improvements by requiring at least one of four justification metrics to have improved: starter value percentile movement (>=2%), expected wins movement (>=0.15), injury health ratio improvement (>=3%), or trade efficiency improvement (>=2%); unjustified climbs are capped at 1 position per week, with rank collisions resolved automatically. Each team's `antiGaming` field exposes constraint status, original vs adjusted rank, per-metric justifications with deltas, and failed metrics. Justification metrics are persisted in `RankingsSnapshot.metricsJson` for week-over-week comparison. Each team also includes: (1) `rankChangeDrivers` — top 3 numeric drivers explaining why rank changed, with human-readable labels, current values, previous-week values, deltas, and units (ppg, percentile, expected wins, etc.); (2) `forwardOdds` — playoff %, top-3 finish %, and title % computed via full Monte Carlo season + playoff bracket simulation (1000 iterations, deterministic round-robin opponents, seeded bracket playoffs); (3) `confidenceBadge` — GOLD/SILVER/BRONZE tier with label and tooltip derived from data quality confidence rating and coverage tier; (4) `rankSparkline` — array of historical rank positions from `RankingsSnapshot` (up to 12 weeks) for sparkline visualization. The POST endpoint returns model confidence, data freshness, weight version metadata, learned params, and segment key.
...
-   **Unified Canonical Context (v1)**: All trade tools (evaluator, analyzer, finder) share a single `TradeDecisionContextV1` via `buildUnifiedTradeContext()` (`lib/trade-engine/unified-context.ts`). This bridges the legacy `LeagueDecisionContext` with the canonical schema, mapping team profiles, asset valuations, needs/surplus, and competitive windows into `TeamSnapshot` format with source freshness and data coverage computed automatically. The evaluator route constructs this canonical context from its existing priced assets and league data, surfacing `canonicalContext` metadata (freshness, coverage, quality) in every API response. `summarizeUnifiedContext()` provides LLM-ready text summaries.
...
-   **Trade Analysis UI Badges** (`components/TradeAnalysisBadges.tsx`): Three expandable badge components display data quality signals inline with trade results: (1) **Freshness Badge** — shows composite data freshness grade (fresh/aging/stale/expired) with expandable per-source breakdown (rosters, valuations, injuries, ADP, analytics, trade history) including age labels; (2) **Coverage Badge** — shows data coverage tier (Full/Partial/Limited) with colored indicators and expandable dimension progress bars (asset coverage, source freshness, data completeness); (3) **Disagreement Badge** — appears when AI models disagree, showing severity (winner mismatch, confidence spread, review mode) with expandable key differences and disagreement codes. Integrated into `DynastyTradeForm` result display.
-   **3-Section Trade Response**: API responses are structured into: (1) **Value Verdict** (fairness grade, value delta, confidence, veto risk, reasons, freshness, data coverage, disagreement, recommendation type); (2) **Viability Verdict** (acceptance likelihood, partner fit, timing fit, league activity, acceptance signals, Rankings Impact, Injury-Adjusted Replacement Value, and Starter vs Bench Impact Delta); and (3) **Action Plan** (best offer assessment, counter baselines, suggested message text).
-   **Rivalry Week Mode** (`lib/rivalry-engine.ts`, `components/RivalryWeekCards.tsx`): A deterministic rivalry scoring engine that computes head-to-head records, trade friction, record proximity, and matchup impact from Sleeper matchup and transaction data. Produces top 3 rivalries with composite scores. Three weekly narrative cards: "Rivalry of the Week" (highest composite score with expandable breakdown), "Revenge Game" (recent loser seeking payback, requires real loss not tie), and "Trade Tension Index" (teams with highest inter-trade frequency, tension score 0-100). AI narrative layer via GPT-4o generates punchy 1-2 sentence hype text for each card. Integrated into transfer preview response and rendered in af-legacy transfer tab.
-   **Migration Proof** (`components/MigrationProof.tsx`): Split-screen "Before → After" comparison showing Sleeper raw snapshot (settings, standings, trades, stats) vs AF AI-enhanced intelligence (rivalry matchups, trade opportunities, waiver priorities, storylines). Deterministic insights derived from transfer preview data.
