...
-   **League Rankings V2 (Team-Level)**: A two-layer team ranking system (Power Rankings and Dynasty Outlook) with five core scores, AI Coach Box, Motivational Framing System, Luck Meter, and Win Window projection. Features data-driven injury penalty (computed per-roster from Sleeper player status + analytics severity), data-driven draft gain percentile (computed from draft picks vs expected slot values), and a calibratable composite weight config system (`lib/rankings-engine/composite-weights.ts`) that loads weights from `data/composite-weights.json` with fallback to hardcoded defaults. The POST endpoint returns model confidence (score + rating + factors), data freshness (grade + per-source breakdown), and weight version metadata.
