...
-   **Market Timing Alerts**: AI-powered buy/sell signal system on the Alerts tab. Uses FantasyCalc dynasty values + 30-day trends for NFL players and `computeAllDevyIntelMetrics()` for college/devy prospects. Signals: STRONG_BUY, BUY, HOLD, SELL, STRONG_SELL computed deterministically from trend percentage, volatility, dynasty premium, age, draft projection, and draft capital. AI narratives generated via GPT-4o with 15-minute cache. Shared types in `lib/types/market-alerts.ts`, API at `/api/market-alerts`, component at `components/MarketTimingAlerts.tsx`. Supports filter by NFL/Devy and position (QB/RB/WR/TE). Devy alerts enriched with live CFBD college stats (passing/rushing/receiving yards, TDs) with 6-hour cache.
-   **Definitive Player Classification Engine**: Multi-source cross-referencing system that definitively classifies every devy player into one of five statuses: `college` (on current CFBD roster), `declared` (not on roster, draft eligible, not yet in NFL), `drafted` (appeared in CFBD draft picks), `nfl_active` (confirmed in Sleeper NFL database), or `returning` (was draft eligible but chose to stay in college). Uses priority-based classification: Sleeper NFL > CFBD Draft Picks > CFBD Roster > Inference from eligibility/class year. Each classification includes `statusSource`, `statusConfidence` (0-100), and `statusUpdatedAt`. CFBD API v2 field format supported (camelCase). Located in `lib/devy-classification.ts`, draft picks fetcher in `lib/cfb-player-data.ts`, runs daily via `/api/admin/devy-sync`.
