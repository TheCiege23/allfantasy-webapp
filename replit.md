...
-   **Trade Improvement AI (Dual-Brain)**: Agentic Grok (multi-turn tool loop with web_search via Serper + x_keyword_search via xAI) + GPT-4o parallel synthesis. Real-time thinking UI with 6 animated phases. Toggle for real-time news (enables/disables tool loop). Feedback system with thumbs up/down that persists to DB (`TradeSuggestionVote`) for authenticated users, in-memory for anonymous. GPT-4o-mini summarizes vote history into a `tradePreferenceProfile` on `AIUserProfile`, injected into the system prompt to personalize future suggestions. Profile auto-refreshes after 6 hours of staleness, requires minimum 3 votes. Files: `app/components/ImproveTradeModal.tsx`, `app/api/instant/improve-trade/route.ts`, `app/api/feedback/trade/route.ts`, `lib/trade-feedback-profile.ts`, `lib/feedback-store.ts`.
